...
public class TweetParser {
     ...
     * training data.
     * <p>
     * The regular expression {@code "[\\W&&[^']]"} matches non-word characters. The
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        try {
            ...
            if (csvColumn >= arr.length || csvColumn < 0) {
                return null;
            } else {
    ...
    /**
     * Given the argument pathToCSVFile and the column that the tweets are in, use
     * the extractColumn and a FileLineIterator to extract every tweet from the CSV.
    ...
    static List<String> csvFileToTweets(String pathToCSVFile, int tweetColumn) {
        ...
        try {
            FileLineIterator line = new FileLineIterator(pathToCSVFile);

            while (line.hasNext()) {
                String s = extractColumn(line.next(), tweetColumn);
                l.add(s);
                l.remove(null);
            }
     ...
     * <p>
     * Hint: use the String split method and the cleanWord helper defined above. You
     * should be splitting on one space of whitespace since words are delimited by
     * spaces.
     *
     * @param sentence - a (non-null) String representing one sentence with no end
     *                 punctuation from a tweet
    ...
    static List<String> parseAndCleanSentence(String sentence) {
        ...
        for (String s : arr) {
            cleanedList.add(cleanWord(s));
            if (cleanWord(s) == null) {
                cleanedList.remove(cleanWord(s));
            }
        }
        return cleanedList;
    }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        String s = removeURLs(tweet);
        List<String> list = sentenceSplit(s);
        List<List<String>> ans = new LinkedList<List<String>>();
        for (String sen : list) {
            List<String> parsed = parseAndCleanSentence(sen);
            ans.add(parsed);
            if (parsed.isEmpty()) {
                ans.remove(parsed);
            }
    ...
    public static List<List<String>> csvFileToTrainingData(String pathToCSVFile, int tweetColumn) {
        ...
        try {
            ...
            for (String s : tweets) {
                ans.addAll(parseAndCleanTweet(s));
            }
        } catch (IllegalArgumentException e) {
            e.printStackTrace();
            throw new IllegalArgumentException("filePath does not exist");
