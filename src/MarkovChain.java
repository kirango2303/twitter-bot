...
public class MarkovChain implements Iterator<String> {
     ...
     * field called chain of type
     * {@code final Map<String, ProbabilityDistribution<String>> }.
     *
    ...
    void addBigram(String first, String second) {
        if (first == null) {
            throw new IllegalArgumentException("First parameter null");
        }
        if (chain.containsKey(first)) {
            ProbabilityDistribution<String> pd1 = chain.get(first);
            pd1.record(second);
        } else {
            ProbabilityDistribution<String> pd2 = new ProbabilityDistribution<String>();
            pd2.record(second);
            chain.put(first, pd2);
        }
    }
    ...
    /**
     * Returns: either 1. the most recent word passed to reset() if reset was
     * previously called 2. a successor picked from the probability distribution
     * associated with the most recently returned from next()
     *
    ...
    public String next() {
        ...
        String nextWord = chain.get(currentWord).pick(ng);
    ...
    public void fixDistribution(List<String> words, boolean recordStart) {
        ...
        if (startWords.count(curWord) < 1) {
            ...
            "first word " + curWord + " not present in " + "startWords");
        }
        ...
        while (words.size() > 0) {
            ...
            if (curDistribution.count(nextWord) < 1) {
                throw new IllegalArgumentException(
                        "word " + nextWord + " not found as a child of" + " word " + curWord);
            }
