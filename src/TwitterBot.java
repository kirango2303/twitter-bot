...
public class TwitterBot {
    ...
    public TwitterBot(String csvFile, int tweetColumn, NumberGenerator ng) {
        ...
        this.ng = ng;
        List<List<String>> l = TweetParser.csvFileToTrainingData(csvFile, tweetColumn);
        for (List<String> s : l) {
            mc.train(s.iterator());
        }
     ...
     * length, you should include it and end the tweet with randomPunctuation(). The
     * resulting tweet may be slightly longer than the desired length (and possibly
    ...
    public String generateTweet(int length) {
        ...
        while (count < length) {
            if (!mc.hasNext() && count == length - 1) {
                String p = randomPunctuation();
                tweet = tweet + p;
                return tweet;
            } else if (!mc.hasNext()) {
                String p = randomPunctuation();
    ...
    public void fixDistribution(List<String> tweet) {
        /*
         * The goal of `fixDistribution` is to ensure that our underlying probability
         * distributions output a tweet in the order that we desire. My implementation
         * does this by splitting us into 2 LNGs: 1. TwitterBot LNG This LNG serves to
         * make sure our punctuation is output in the order we expect. 2. MarkovChain
         * LNG This LNG makes sure the tweets are output in the proper order. This will
         * be built by running `fixDistribution` on the Markov Chain with punctuation
         * replaced by null.
         */
